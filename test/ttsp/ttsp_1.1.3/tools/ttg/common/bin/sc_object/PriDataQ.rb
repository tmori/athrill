...
require "ttc/bin/sc_object/PriDataQ.rb"
...
module CommonModule
  ...
  class PriDataQ < SCObject
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_P_DATAQUEUE, aPath, bIsPre)

      @sRefAPI       = FNC_REF_PDQ
      @sRefSWaitAPI  = FNC_REF_SWAIT_PDQ
      @sRefRWaitAPI  = FNC_REF_RWAIT_PDQ
      @sRefStrType   = TYP_T_TTSP_RPDQ
      @sRefStrVar    = GRP_VAR_TYPE[TYP_T_TTSP_RPDQ]
      @sRefAtr       = STR_PDQATR
      @sRefDataCnt   = STR_PDQCNT
      @sRefDataList  = STR_SPDQCNT
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_CRE_PDQ}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_DATACNT]}, #{@hState[TSR_PRM_MAXDPRI]}, NULL});", @hState[TSR_PRM_CLASS]) # [IMCodeElement]優先度データキューを生成する静的API
    end
  end
end
