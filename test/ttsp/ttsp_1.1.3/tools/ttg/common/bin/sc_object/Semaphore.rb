...
require "ttc/bin/sc_object/Semaphore.rb"
...
module CommonModule
  ...
  class Semaphore < SCObject
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_SEMAPHORE, aPath, bIsPre)

      @sRefAPI      = FNC_REF_SEM
      @sRefRWaitAPI = FNC_REF_WAIT_SEM
      @sRefStrType  = TYP_T_TTSP_RSEM
      @sRefStrVar   = GRP_VAR_TYPE[TYP_T_TTSP_RSEM]
      @sRefAtr      = STR_SEMATR
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_CRE_SEM}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_ISEMCNT]}, #{@hState[TSR_PRM_MAXSEM]}});", @hState[TSR_PRM_CLASS]) # [IMCodeElement]セマフォを生成する静的API
    end
  end
end
