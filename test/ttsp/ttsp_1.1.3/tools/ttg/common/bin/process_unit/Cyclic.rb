...
require "ttc/bin/process_unit/Cyclic.rb"
...
module CommonModule
  ...
  class Cyclic < ProcessUnit
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_CYCLE, aPath, bIsPre)

      @sRefAPI     = FNC_REF_CYC
      @sRefStrType = TYP_T_TTSP_RCYC
      @sRefStrVar  = GRP_VAR_TYPE[TYP_T_TTSP_RCYC]
      @sRefState   = STR_CYCSTAT
      @sRefExInf   = STR_EXINF
      @sRefPrcID   = STR_PRCID
      @sRefAtr     = STR_CYCATR
    end
    ...
    def gc_config(cElement)
      ...
      if (@cConf.is_timer_local?())
        cElement.set_config("#{API_CRE_CYC}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_EXINF]}, #{@sObjectID.downcase}, #{@hState[TSR_PRM_CYCTIM]}, #{@hState[TSR_PRM_CYCPHS]}});", @hState[TSR_PRM_CLASS]) # [IMCodeElement] 周期ハンドラを生成する静的API
      else
        cElement.set_config("#{API_CRE_CYC}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_EXINF]}, #{@sObjectID.downcase}, #{@hState[TSR_PRM_CYCTIM]}, #{@hState[TSR_PRM_CYCPHS]}});", @cConf.get_time_manage_class()) # [IMCodeElement] 周期ハンドラを生成する静的API
      end
    end

    #=================================================================
    # 概  要: cycatrとcycstatのref処理のみをcElementに追加する
    #=================================================================
    def gc_obj_ref_only_cyc(cElement, hProcUnitInfo)
      check_class(IMCodeElement, cElement) # エレメント
      check_class(Hash, hProcUnitInfo)     # 処理単位情報

      cElement.set_local_var(hProcUnitInfo[:id], @sRefStrVar, @sRefStrType)
      cElement.set_code(hProcUnitInfo, "", false)
