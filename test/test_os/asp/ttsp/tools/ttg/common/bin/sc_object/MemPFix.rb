...
require "ttc/bin/sc_object/MemPFix.rb"
...
module CommonModule
  ...
  class MemPFix < SCObject
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_MEMORYPOOL, aPath, bIsPre)

      @sRefAPI      = FNC_REF_MPF
      @sRefRWaitAPI = FNC_REF_WAIT_MPF
      @sRefStrType  = TYP_T_TTSP_RMPF
      @sRefStrVar   = GRP_VAR_TYPE[TYP_T_TTSP_RMPF]
      @sRefAtr      = STR_MPFATR
      @sRefWaitCnt  = STR_WAITCNT
    end
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_CRE_MPF}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_BLKCNT]}, #{@hState[TSR_PRM_BLKSZ]}, NULL, NULL});", @hState[TSR_PRM_CLASS]) # [IMCodeElement]固定長メモリプールを生成する静的API
    end
  end
end
