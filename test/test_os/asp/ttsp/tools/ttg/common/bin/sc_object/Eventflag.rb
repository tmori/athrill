...
require "ttc/bin/sc_object/Eventflag.rb"
...
module CommonModule
  ...
  class Eventflag < SCObject
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_EVENTFLAG, aPath, bIsPre)

      @sRefAPI       = FNC_REF_FLG
      @sRefRWaitAPI  = FNC_REF_RWAIT_FLG
      @sRefStrType   = TYP_T_TTSP_RFLG
      @sRefStrVar    = GRP_VAR_TYPE[TYP_T_TTSP_RFLG]
      @sRefAtr       = STR_FLGATR
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_CRE_FLG}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_IFLGPTN]}});", @hState[TSR_PRM_CLASS]) # [IMCodeElement]イベントフラグを生成する静的API
    end
  end
end
