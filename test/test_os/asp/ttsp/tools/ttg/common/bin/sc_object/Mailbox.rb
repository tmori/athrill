...
require "ttc/bin/sc_object/Mailbox.rb"
...
module CommonModule
  ...
  class Mailbox < SCObject
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_MAILBOX, aPath, bIsPre)

      @sRefAPI      = FNC_REF_MBX
      @sRefRWaitAPI = FNC_REF_RWAIT_MBX
      @sRefStrType  = TYP_T_TTSP_RMBX
      @sRefStrVar   = GRP_VAR_TYPE[TYP_T_TTSP_RMBX]
      @sRefAtr      = STR_MBXATR
      @sRefMsgCnt   = STR_MSGCNT
      @sRefMaxMPri  = STR_MAXMPRI
      @sRefWaitCnt  = STR_RWAITCNT
    end
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_CRE_MBX}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_MAXMPRI]}, NULL});", @hState[TSR_PRM_CLASS]) # [IMCodeElement]メールボックスを生成する静的API
    end
  end
end
