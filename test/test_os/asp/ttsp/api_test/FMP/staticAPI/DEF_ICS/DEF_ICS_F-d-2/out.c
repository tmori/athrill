...
void main_task(intptr_t exinf){
	syslog_0(LOG_NOTICE,"FMP_staticAPI_DEF_ICS_F_d_2: Start");

	ttsp_mp_check_point(1, 1);

	ttsp_cpuexc_raise(TTSP_EXCNO_A);

	ttsp_mp_wait_check_point(1, 3);
	ttsp_mp_wait_check_point(2, 2);

	syslog_0(LOG_NOTICE,"FMP_staticAPI_DEF_ICS_F_d_2: OK");

	ttsp_mp_check_finish(1, 4);
}
...
void exc1(void* p_excinf){
	...
	check_assert(&ercd < (ER*)((char *)_kernel_istk_table[0] + TTSP_NON_TASK_STACK_SIZE));	/* ローカル変数がスタックの領域内にあるか確認 */

	ttsp_mp_check_point(1, 3);
}
void exc2(void* p_excinf){
	ER ercd;
	...
	check_assert(TTSP_NON_TASK_STACK_SIZE == _kernel_istksz_table[1]);
	check_assert(&ercd > (ER*)((char *)_kernel_istk_table[1]));								/* ローカル変数がスタックの領域内にあるか確認 */
	check_assert(&ercd < (ER*)((char *)_kernel_istk_table[1] + TTSP_NON_TASK_STACK_SIZE));	/* ローカル変数がスタックの領域内にあるか確認 */
