 ...
 *  CPU例外を発生させる命令
 */
#define RAISE_CPU_EXCEPTION Asm("trap");
 ...
 *  タスクのスタックサイズ
 */
#define TTSP_TASK_STACK_SIZE  40960
...
/*  
 *  割込み番号(正常値)
 *  ttsp_int_raise関数では，割込み番号が7から連番であることを想定したコードになっているので注意
 */
#define TTSP_INTNO_A      0x10007	/* PE1割込み番号A */
#define TTSP_INTNO_B      0x10008	/* PE1割込み番号B */
#define TTSP_INTNO_C      0x10009	/* PE1割込み番号C */

#define TTSP_INTNO_PE2_A  0x20007	/* PE2割込み番号A */
#define TTSP_INTNO_PE2_B  0x20008	/* PE2割込み番号B */
#define TTSP_INTNO_PE2_C  0x20009	/* PE2割込み番号C */

#define TTSP_INTNO_PE3_A  0x30007	/* PE3割込み番号A */
#define TTSP_INTNO_PE3_B  0x30008	/* PE3割込み番号B */
#define TTSP_INTNO_PE3_C  0x30009	/* PE3割込み番号C */

#define TTSP_INTNO_PE4_A  0x40007	/* PE4割込み番号A */
#define TTSP_INTNO_PE4_B  0x40008	/* PE4割込み番号B */
#define TTSP_INTNO_PE4_C  0x40009	/* PE4割込み番号C */
...
#define TTSP_GLOBAL_IRC_INTNO_A  0x00009	/* グローバルIRC用割込み番号A(サポート外) */
#define TTSP_GLOBAL_IRC_INTNO_B  0x0000A	/* グローバルIRC用割込み番号B(サポート外) */
#define TTSP_GLOBAL_IRC_INTNO_C  0x0000B	/* グローバルIRC用割込み番号C(サポート外) */
#define TTSP_GLOBAL_IRC_INTNO_D  0x0000C	/* グローバルIRC用割込み番号D(サポート外) */
#define TTSP_GLOBAL_IRC_INTNO_E  0x0000D	/* グローバルIRC用割込み番号E(サポート外) */
#define TTSP_GLOBAL_IRC_INTNO_F  0x0000E	/* グローバルIRC用割込み番号F(サポート外) */
...
#define TTSP_GLOBAL_IRC_INHNO_A  TTSP_GLOBAL_IRC_INTNO_A	/* グローバルIRC用割込みハンドラ番号A(サポート外) */
#define TTSP_GLOBAL_IRC_INHNO_B  TTSP_GLOBAL_IRC_INTNO_B	/* グローバルIRC用割込みハンドラ番号B(サポート外) */
#define TTSP_GLOBAL_IRC_INHNO_C  TTSP_GLOBAL_IRC_INTNO_C	/* グローバルIRC用割込みハンドラ番号C(サポート外) */

/*
 *  CPU例外ハンドラ番号(正常値)
 */
#define TTSP_EXCNO_A      0x10000	/* CPU例外発生元のコンテキストへreturn可能(未定義命令) */
#define TTSP_EXCNO_B      0x10002	/* 本番号でCPU例外を発生させるテストケースはない(SWI) */
#define TTSP_EXCNO_PE2_A  0x20000	/* PE2未定義命令 */
 ...
 */
#define TTSP_SIL_DLY_NSE_TIME  20000
