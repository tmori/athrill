 ...
 *		する．ディスパッチ禁止状態で実施する．
 *	(C-1)
 *		中優先度タスク（TASK1）に高優先度上限ミューテックス（MTX2）をロッ
 *		クさせ，別の中優先度タスク（TASK2）がMTX2を待っている状態で，
 *		TASK1を一時的に高優先度にし，さらにTASK1にMTX2をロック解除させ，
 *		TASK2の優先度が高優先度になることを確認する．また，TASK2の優先
 ...
 *		ことを確認する．
 *	(C-3)
 *		中優先度タスク（TASK1）に高優先度上限ミューテックス（MTX3）と中
 *		優先度上限ミューテックス（MTX1）をロックさせ，別の中優先度タス
 ...
 *	== TASK2（続き）==
 *	60:	unl_mtx(MTX1)
 *	//		高：TASK2，中：TASK1→TASK3，MTX2：TASK2
 *	61:	unl_mtx(MTX2)
 *	//		中：TASK2→TASK1→TASK3
 *
 *	62:	loc_mtx(MTX1)
