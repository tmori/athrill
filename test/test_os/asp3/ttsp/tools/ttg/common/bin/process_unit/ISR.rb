...
require "ttc/bin/process_unit/ISR.rb"
...
module CommonModule
  ...
  class ISR < ProcessUnit
    ...
    def initialize(sObjectID, hObjectInfo, aPath, bIsPre)
      ...
      super(sObjectID, hObjectInfo, TSR_OBJ_ISR, aPath, bIsPre)
    end
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_ATT_ISR}({#{KER_TA_NULL}, #{@hState[TSR_PRM_EXINF]}, #{@hState[TSR_PRM_INTNO]}, #{@sObjectID.downcase}, #{@hState[TSR_PRM_ISRPRI]}});", @hState[TSR_PRM_CLASS])
    end

    #=================================================================
    # 概  要: 拡張情報をassertチェックするコードを返す
    #=================================================================
    def gc_assert_exinf(cElement, hProcUnitInfo)
      check_class(IMCodeElement, cElement) # エレメント
