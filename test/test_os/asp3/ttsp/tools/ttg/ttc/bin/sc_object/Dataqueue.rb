  ...
  class Dataqueue < SCObject
    ...
    def object_check(bIsPre)
      ...
      if (@hState[TSR_PRM_DATACNT] > nDataCount && !@hState[TSR_PRM_STSKLIST].nil?() && !@hState[TSR_PRM_STSKLIST].empty?())
        aErrors.push(YamlError.new("T3_DTQ001: " + ERR_SEND_WAITING_HAVE_SPACE, @aPath))
      end
      ### T3_DTQ002: 管理領域にデータを持っているのに受信待ちタスクがいる
      if (nDataCount > 0 && !@hState[TSR_PRM_RTSKLIST].nil?() && !@hState[TSR_PRM_RTSKLIST].empty?())
        aErrors.push(YamlError.new("T3_DTQ002: " + ERR_RECV_WAITING_HAVE_DATA, @aPath))
      end
      ### T3_DTQ003: 管理領域以上のデータを持っている
      if (nDataCount > @hState[TSR_PRM_DATACNT])
        aErrors.push(YamlError.new("T3_DTQ003: " + ERR_DATALIST_HAVE_OVER_DATA, @aPath))
      end
    ...
    def complement_init_object_info()
      ...
      # dtqatr
      unless (is_specified?(TSR_PRM_DTQATR))
        @hState[TSR_PRM_ATR] = "ANY_ATT_DTQ"
      end
      # dtqcnt
      unless (is_specified?(TSR_PRM_DTQCNT))
