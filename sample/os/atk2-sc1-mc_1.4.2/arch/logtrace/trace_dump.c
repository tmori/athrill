 ...
 *  の責任を負わない．
 *
 *  $Id: trace_dump.c 2401 2017-03-14 09:09:24Z witz-itoyo $
 */
...
typedef enum {
	...
	PR_UNIT_STAOSAP,    /* OSAP固有スタートアップフック */
	PR_UNIT_ERROSAP,    /* OSAP固有エラーフック */
	PR_UNIT_SHUTOSAP,   /* OSAP固有シャットダウンフック */
	PR_UNIT_INSHUTAC,   /* OS内部からのShutdownAllCoresの呼び出し */
...
static const char8 *trace_print_mcenter(const TRACE *trace, uintptr info[]);
static const char8 *trace_print_mcleave(const TRACE *trace, uintptr info[]);
static const char8 *trace_print_spnenter(const TRACE *trace, uintptr info[]);
static const char8 *trace_print_spnleave(const TRACE *trace, uintptr info[]);
...
trace_print_osapleave(const TRACE *trace, uintptr info[])
{
	...
	switch (type) {
	case TFN_GETAPPLICATIONID:
		info[0] = (uintptr) kernel_osapid_str((ApplicationType) trace->loginfo[1]);
		info[1] = trace->loginfo[1];
...
trace_print_iocenter(const TRACE *trace, uintptr info[])
{
	...
	case TFN_IOC_RECEIVE_GENERIC:
		info[0] = (uintptr) kernel_iocid_str((IocType) trace->loginfo[1]);
		info[1] = trace->loginfo[1];
...
trace_print(const TRACE *p_trace, void (*outputc)(char8 c))
{
	...
	case LOG_TYPE_INSHUTALLCORES | LOG_ENTER:
		trace_push_context(PR_UNIT_INSHUTAC, 0U);
