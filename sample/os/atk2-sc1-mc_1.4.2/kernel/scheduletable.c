 ...
 *  の責任を負わない．
 *
 *  $Id: scheduletable.c 2401 2017-03-14 09:09:24Z witz-itoyo $
 */
...
schtbl_initialize(void)
{
	...
	for (i = 0U; i < tnum_scheduletable; i++) {
		if ((schtblinib_table[i].p_cntcb->p_cntinib != NULL) &&
			(schtblinib_table[i].p_cntcb->p_cntinib->coreid == coreid)) {
			p_schtblcb = p_schtblcb_table[i];
...
StartScheduleTableRel(ScheduleTableType ScheduleTableID, TickType Offset)
{
  ...
  errorhook_start:
	...
	get_my_p_ccb()->temp_errorhook_par2.offset = Offset;
...
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->temp_errorhook_par1.schtblid = ScheduleTableID;
	get_my_p_ccb()->temp_errorhook_par2.start = Start;
#endif /* CFG_USE_PARAMETERACCESS */
...
StopScheduleTable(ScheduleTableType ScheduleTableID)
{
	...
	p_ccb = get_p_ccb(p_schtblcb->p_schtblinib->p_cntcb->p_cntinib->coreid);
	x_nested_lock_os_int();
	acquire_cnt_lock(p_ccb);
				  ...
				  ScheduleTableType ScheduleTableID_To)
{
	...
	CHECK_CORE(p_curcb->p_schtblinib != NULL);
	CHECK_CORE(p_nextcb->p_schtblinib != NULL);
	CHECK_ID(p_curcb->p_schtblinib->p_cntcb == p_nextcb->p_schtblinib->p_cntcb);

	p_ccb = get_p_ccb(p_curcb->p_schtblinib->p_cntcb->p_cntinib->coreid);
	x_nested_lock_os_int();
	acquire_cnt_lock(p_ccb);
  ...
  errorhook_start:
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->temp_errorhook_par1.schtblid_from = ScheduleTableID_From;
	get_my_p_ccb()->temp_errorhook_par2.schtblid_to = ScheduleTableID_To;
#endif /* CFG_USE_PARAMETERACCESS */
...
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->temp_errorhook_par1.schtblid = ScheduleTableID;
	get_my_p_ccb()->temp_errorhook_par2.p_schtblstate = ScheduleStatus;
#endif /* CFG_USE_PARAMETERACCESS */
...
schtbl_exppoint_process(SCHTBLCB *p_schtblcb, const CNTCB *p_cntcb)
{
	...
	p_currentosap_saved = p_ccb->p_currentosap;
	p_ccb->p_currentosap = p_schtblcb->p_schtblinib->p_osapinib;

	(p_exppoint->expptfnt)();

	p_ccb->p_currentosap = p_currentosap_saved;
