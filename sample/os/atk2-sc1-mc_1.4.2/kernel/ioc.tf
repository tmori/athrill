...
$  の責任を負わない．
$
$  $Id: ioc.tf 2401 2017-03-14 09:09:24Z witz-itoyo $
$
...
$FOREACH iocsndid IOCSND.ID_LIST$
	...
	$IF !EQ(IOC.BUFLEN[iocid], "NULL")$
		$new_iocsnd_list = APPEND(new_iocsnd_list,iocsndid)$
	$END$
$END$
$FOREACH iocsndid IOCSND.ID_LIST$
	$iocid = IOCSND.IOC[iocsndid]$
	...
	$END$
$END$
$IOCSND.ID_LIST = new_iocsnd_list$
...
$		// キューあり
		$IF !EQ(IOC.BUFLEN[iocid], "NULL")$
			$FOREACH iocsnd IOC.IOCSND_LIST[iocid]$
				$tnum_ioc_wrapper_send = tnum_ioc_wrapper_send +1$
			$END$
		$END$
	$END$
...
$END$
	...
	#define TNUM_IOC_WRAPPER_SEND 	UINT_C($tnum_ioc_wrapper_send$)$NL$
...
$FILE "Os_Lcfg.c"$
	...
	/****** Object IOC ******/$NL$
	const IocType	kernel_tnum_ioc = TNUM_IOC;$NL$
	const IocType	kernel_tnum_queueioc = TNUM_QUEUEIOC;$NL$
	$NL$
...
$ 				// ここには来ない（エラーチェック処理で検出済み）
			...
			$ELSE$
				const IOCMB_$iocid$ ioc_inival_$iocid$ = {$NL$
...
$ 	// IOC初期化値テーブル
	void * const kernel_ioc_inival_table[TNUM_IOC] = {$NL$
...
$ 	// IOC管理ブロックテーブル
	$IF tnum_queioc > 0$
		IOCCB kernel_ioccb_table[TNUM_QUEUEIOC];$NL$
	$ELSE$
		TOPPERS_EMPTY_LABEL(IOCCB, kernel_ioccb_table);$NL$
	$END$
	$NL$
...
$ 	// IOCデータ管理ブロックテーブルの宣言
	$FOREACH iocid IOC.ID_LIST$
		$IF EQ(IOC.BUFLEN[iocid], "NULL")$
			IOCMB_$iocid$ kernel_iocmb_table_$iocid$[1];$NL$
		$ELSE$
			IOCMB_$iocid$ kernel_iocmb_table_$iocid$[$IOC.BUFLEN[iocid]$];$NL$
		$END$
	$END$
	$NL$

$ 	// IOC初期化テーブル
	const IOCINIB kernel_iocinib_table[TNUM_IOC] = {$NL$
	$JOINEACH iocid IOC.ID_LIST ",\n"$
		$TAB${$NL$
...
$ 			// コア間通信かどうかを確認する
			$FOREACH iocsnd IOC.IOCSND_LIST[iocid]$
				$FOREACH iocsndid IOC.IOCSND_LIST[iocid]$
$ 					// Senderと他のSenderがコア間通信かどうか
					$IF !EQ(OSAP.CORE[IOCSND.OSAPID[iocsnd]], OSAP.CORE[IOCSND.OSAPID[iocsndid]])$
						$find = 1$
					$END$
				$END$
$ 				// SenderとReceiverがコア間通信かどうか
				$IF !EQ(OSAP.CORE[IOCSND.OSAPID[iocsnd]], OSAP.CORE[IOCRCV.OSAPID[IOC.IOCRCV[iocid]]])$
					$find = 1$
				$END$
			$END$
			$IF find$
			...
			$TAB$$TAB$&kernel_osapinib_table[$IOCRCV.OSAPID[IOC.IOCRCV[iocid]]$],$NL$
			$TAB$$TAB$&kernel_iocmb_table_$iocid$$NL$
		$TAB$}
	$END$
	$NL$};$NL$
	$NL$
...
$ 	// IOCラッパー初期化テーブル
	const IOCWRPINIB kernel_iocwrpinib_table[TNUM_IOC_WRAPPER] = {$NL$
	$JOINEACH iocsndid IOCSND.ID_LIST ",\n"$
		$TAB${$NL$
			$TAB$$TAB$&kernel_iocinib_table[$IOCSND.IOC[iocsndid]$],$NL$
			$TAB$$TAB$&kernel_osapinib_table[$IOCSND.OSAPID[iocsndid]$],$NL$
			$IF EQ(IOCSND.SENDERID[iocsndid], "NULL")$
				$TAB$$TAB$0U$NL$
			$ELSE$
	...
	$NL$
$ELSE$
	TOPPERS_EMPTY_LABEL(void * const, kernel_ioc_inival_table);$NL$
	TOPPERS_EMPTY_LABEL(IOCCB, kernel_ioccb_table);$NL$
	TOPPERS_EMPTY_LABEL(const IOCINIB, kernel_iocinib_table);$NL$
	TOPPERS_EMPTY_LABEL(const IOCWRPINIB, kernel_iocwrpinib_table);$NL$

$END$

	const IocType kernel_tnum_ioc_wrapper_send = TNUM_IOC_WRAPPER_SEND;$NL$
	const IocType kernel_tnum_ioc_wrapper= TNUM_IOC_WRAPPER;$NL$
					...
					$IF !EQ(IOCSND.IMPLKIND[iocsnd], "MACRO")$
						...
						{$NL$
							...
							$TAB$in.data1 =$SPC$
								...
								$END$
							in1;$NL$
							$TAB$ercd = $iocsender_generic$(IOC_WRAPPER_$+wrapper_count - 1$, (void *)(&in));$NL$
							$wrapper_count = wrapper_count + 1$
							$TAB$return(ercd);$NL$
						}$NL$
					$ELSE$
						$FILE "Ioc.h"$
						$IF ISFUNCTION("DEFINE_IOC_MACRO")$
							$DEFINE_IOC_MACRO(FORMAT("%s_%s", iocsender, iocid))$
						$ELSE$
...
$					// FUNCTION, MACRO, DO_NOT_CARE, 省略(NULL)以外のエラーチェックは確認済み
					...
					$ELSE$
						$FILE "Ioc.h"$
						$IF ISFUNCTION("DEFINE_IOC_MACRO")$
							$DEFINE_IOC_MACRO(FORMAT("%s_%s_%s", iocsender, iocid, IOCSND.SENDERID[iocsnd]))$
						$ELSE$
							LOCAL_INLINE Std_ReturnType$NL$
							$iocsender$_$iocid$_$IOCSND.SENDERID[iocsnd]$
						$END$
...
$			// FUNCTION, MACRO, DO_NOT_CARE, 省略(NULL)以外のエラーチェックは確認済み
			...
			$ELSE$
				$FILE "Ioc.h"$
				$IF ISFUNCTION("DEFINE_IOC_MACRO")$
					$DEFINE_IOC_MACRO(FORMAT("%s_%s", iocreceiver, iocid))$
				$ELSE$
					LOCAL_INLINE Std_ReturnType$NL$
					$iocreceiver$_$iocid$
				$END$
