 ...
 *  の責任を負わない．
 *
 *  $Id: t_perror.c 2401 2017-03-14 09:09:24Z witz-itoyo $
 */
...
void
t_perror(uint32 prio, const char8 *file, sint32 line, const char8 *expr, StatusType ercd)
{
	syslog_5(prio, "%s (%d) reported by '%s' in line %d of '%s'.",
			 atk2_strerror(ercd), ercd, expr, line, file);

}

void
t_ioc_perror(uint32 prio, const char8 *file, sint32 line, const char8 *expr, StatusType ercd)
{
	syslog_5(prio, "%s (%d) reported by '%s' in line %d of '%s'.",
			 atk2_ioc_strerror(ercd), ercd, expr, line, file);
