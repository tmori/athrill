 ...
 *  の責任を負わない．
 *
 *  $Id: strerror.c 2401 2017-03-14 09:09:24Z witz-itoyo $
 */
...
const char8 *
atk2_strerror(StatusType ercd)
{
	/* エラー文字列テーブル	*/
	static const char8	*ercd_tbl[] = {
		"E_OK",
		"E_OS_ACCESS",
		"E_OS_CALLEVEL",
		"E_OS_ID",
		...
		"E_OS_TIMEINSUFFICIENT",
		"E_OS_PROTECTION_TIMEWINDOW",
		"E_OS_PROTECTION_COUNT_ISR",
		"unknown error"
	};

	StatusType			error;

	if (ercd > (StatusType) ERRCODE_NUM) {
		error = (StatusType) (ERRCODE_NUM + 1U);
	}
	else {
		error = ercd;
	}
	return(ercd_tbl[error]);
}


const char8 *
atk2_ioc_strerror(StatusType ercd)
{
	char8 *str;
