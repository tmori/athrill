 ...
 *  の責任を負わない．
 *
 *  $Id: t_stdlib.h 2401 2017-03-14 09:09:24Z witz-itoyo $
 */
 ...
 *  エラーメッセージ文字列を返す関数
 *
 *  ercdで指定されたエラーコードのメインエラーコードの名称（E_OS_ID，
 *  E_OS_VALUEなど）の文字列を返す関数
 */
extern const char8 *atk2_strerror(StatusType ercd);

/*
 *  エラーメッセージ文字列を返す関数(IOC)
 */
 ...
 *  ンエラーコード等を表示するための関数
 */
extern void	t_perror(uint32 prio, const char8 *file, sint32 line,
					 const char8 *expr, StatusType ercd);

/*
 *  エラーメッセージのログ出力(IOC)
 */
extern void	t_ioc_perror(uint32 prio, const char8 *file, sint32 line,
						 const char8 *expr, StatusType ercd);
