 ...
 *  の責任を負わない．
 *
 *  $Id: prc_config.h 540 2015-12-29 01:00:01Z ertl-honda $
 */
...
/*
 * <TRACE> (6-12-1-3) CHECK_FUNC_ALIGN（オプション）	処理単位のアライン単位
 */
...
/*
 * <TRACE> (6-12-1-4) CHECK_FUNC_NONNULL（オプション）処理単位の非NULLチェック
 */
...
/*
 * <TRACE> (6-12-2-2) StackType *TOPPERS_OSTKPT(sint8 *stk, MemorySizeType stksz)
 *
 ...
 * C1ISRのネスト回数のカウント
 */
extern uint32		c1isr_nest_cnt;

/*
 *  カーネル管理内（カテゴリ2）の割込みを禁止するためのISPRの設定値
 */
extern const uint8	ispr_isr2_mask;

/*
 *  カーネル管理外（カテゴリ1）の割込みとなる優先度のISPRのビット
 */
extern const uint8	ispr_isr1_mask;

/*
* OS割り込み禁止の割り込み優先度の値(CPU割込み優先度)
 */
extern const uint8	c2isr_iintpri;
...
/*
 * 割込み優先度の値(CPU割込み優先度)をOS割り禁レベルに設定する
 */
extern void set_intpri_lock_os_int(void);

/*
 * 割込み優先度の値(CPU割込み優先度)を元に戻す
 */
...
/*
 *  [CPU割込み優先度]と[ATK2の割り込み優先度(内部表現)]の変更
 */
#ifndef TOPPERS_MACRO_ONLY
#define EXT_IPM(iipm)	((PriorityType) ((iipm) - (TNUM_INTPRI)))     /* [CPU割込み優先度]を[ATK2の割り込み優先度(内部表現)]に */
#define INT_IPM(ipm)	((uint32) ((ipm) + (TNUM_INTPRI)))            /* [ATK2の割り込み優先度(内部表現)]を[CPU割込み優先度]に */
#else /* TOPPERS_MACRO_ONLY */
#define EXT_IPM(iipm)	((iipm) - (TNUM_INTPRI))             			/* [CPU割込み優先度]を[ATK2の割り込み優先度(内部表現)]に */
#define INT_IPM(ipm)	((ipm) + (TNUM_INTPRI))              			/* [ATK2の割り込み優先度(内部表現)]を[CPU割込み優先度]に */
...
/*
 * <TRACE> (6-3-1-1) void x_lock_all_int(void)
 *
 *  全割込み禁止状態への移行
 */
LOCAL_INLINE void
...
/*
 * <TRACE> (6-3-1-2) void x_unlock_all_int(void)
 *
 *  全割込み禁止状態の解除
 */
LOCAL_INLINE void
...
/*
 * <TRACE> (6-3-3-1) void x_nested_lock_os_int(void)
 *
 *  OS割込み禁止
 *  API実行時に呼び出される
...
x_nested_lock_os_int(void)
{
	...
	if (nested_lock_os_int_cnt == 0U) {
		set_intpri(c2isr_iintpri);   /* OS割り込み禁止に設定 */
	}
...
/*
 * <TRACE> (6-3-3-3) void x_nested_unlock_os_int(void)
 *
 *  OS割込み解除
 *  API実行時に呼び出される
 *  割込み優先度マスクがISR2の優先度マスクの範囲より高い状態で呼び出される
...
/*
 * <TRACE> (6-3-3-2) void x_suspend_lock_os_int(void)
 *
...
x_suspend_lock_os_int(void)
{
	...
	ispr = sil_reb_mem((void*)INTC_ISPR);

	if ((ispr & ispr_isr1_mask) == 0U) {
		/*
...
/*
 * <TRACE> (6-3-3-4) void x_resume_unlock_os_int(void)
 *
...
/*
 * <TRACE> (6-4-1-1) void x_set_ipm(PriorityType intpri)
 *
 *  (モデル上の)割込み優先度マスクの設定
 *
 *  本OSでは次の二点が成り立つ
...
/*
 * <TRACE> (6-4-1-2) PriorityType x_get_ipm(void)
 *
 *  (モデル上の)割込み優先度マスクの参照
 *
 *  本OSでは次の点が成り立つ
...
/*
 * <TRACE> (6-4-3-1) void x_disable_int(InterruptNumberType intno)
 *
 *  割込み要求禁止フラグのセット
 *
...
/*
 * <TRACE> (6-4-3-2) void x_enable_int(InterruptNumberType intno)
 *
 *  割込み要求禁止フラグの解除
 *
 *  割込み属性が設定されていない割込み要求ラインに対して割込み要求禁止
