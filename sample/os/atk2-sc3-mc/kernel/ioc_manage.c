...
ioc_send_generic(IocType IocWrapperId, const void *in)
{
	...
	CHECK_IOC_ACCESS(p_iocwrpinib->p_sndosapcb == get_my_p_ccb()->p_runosap);
  ...
  errorhook_start:
	...
	get_my_p_ccb()->_errorhook_par1.iocid = IOCID(p_iocwrpinib);
	get_my_p_ccb()->_errorhook_par2.senderid = p_iocwrpinib->senderid;
...
ioc_receive_generic(IocType IocId, void *out)
{
	...
	CHECK_IOC_ACCESS(p_iocinib->p_rcvosapcb == get_my_p_ccb()->p_runosap);
  ...
  errorhook_start:
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->_errorhook_par1.iocid = IocId;
#endif /* CFG_USE_PARAMETERACCESS */
	if (p_iocinib->groupflg != FALSE) {
