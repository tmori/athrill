 ...
 *  の責任を負わない．
 *
 *  $Id: counter_manage.c 425 2015-12-07 08:06:19Z witz-itoyo $
 */
...
IncrementCounter(CounterType CounterID)
{
	...
	CHECK_CORE(p_cntcb->p_cntinib->coreid == x_core_id());
...
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->_errorhook_par1.cntid = CounterID;
	get_my_p_ccb()->_errorhook_par2.p_val = Value;
#endif /* CFG_USE_PARAMETERACCESS */
...
GetElapsedValue(CounterType CounterID, TickRefType Value, TickRefType ElapsedValue)
{
  ...
  errorhook_start:
	...
	get_my_p_ccb()->_errorhook_par1.cntid = CounterID;
	get_my_p_ccb()->_errorhook_par2.p_val = Value;
	get_my_p_ccb()->_errorhook_par3.p_eval = ElapsedValue;
#endif /* CFG_USE_PARAMETERACCESS */
...
incr_counter_process(CNTCB *p_cntcb, CounterType CounterID)
{
	...
	if (p_ccb->p_runisr == NULL) {
		p_cntcb->p_prevcntcb = p_ccb->p_runtsk->p_lastcntcb;
		p_ccb->p_runtsk->p_lastcntcb = p_cntcb;
	}

	expire_process(p_cntcb, CounterID);

	if (p_ccb->p_runisr == NULL) {
		p_ccb->p_runtsk->p_lastcntcb = p_cntcb->p_prevcntcb;
	}
...
incr_counter_action(OSAPCB *p_expire_osapcb, CounterType CounterID)
{
	...
	D_CHECK_ACCESS((p_osapcb->osap_stat == APPLICATION_ACCESSIBLE) || (p_osapcb == p_expire_osapcb));
