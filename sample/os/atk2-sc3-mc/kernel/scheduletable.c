 ...
 *  の責任を負わない．
 *
 *  $Id: scheduletable.c 485 2015-12-17 08:21:50Z witz-itoyo $
 */
...
StartScheduleTableRel(ScheduleTableType ScheduleTableID, TickType Offset)
{
  ...
  errorhook_start:
	...
	get_my_p_ccb()->_errorhook_par2.offset = Offset;
...
StopScheduleTable(ScheduleTableType ScheduleTableID)
{
	...
	if (p_schtblcb->status == SCHEDULETABLE_NEXT) {
		...
		delete_cnt_expr_que(&(p_schtblcb->cntexpinfo), p_cntcb);
	}
				  ...
				  ScheduleTableType ScheduleTableID_To)
{
  ...
  errorhook_start:
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->_errorhook_par1.schtblid_from = ScheduleTableID_From;
	get_my_p_ccb()->_errorhook_par2.schtblid_to = ScheduleTableID_To;
#endif /* CFG_USE_PARAMETERACCESS */
...
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->_errorhook_par1.schtblid = ScheduleTableID;
	get_my_p_ccb()->_errorhook_par2.p_schtblstate = ScheduleStatus;
#endif /* CFG_USE_PARAMETERACCESS */
...
schtbl_exppoint_process(SCHTBLCB *p_schtblcb, const CNTCB *p_cntcb)
{
	...
	p_ccb->p_runosap = p_schtblcb->p_schtblinib->p_osapcb;
