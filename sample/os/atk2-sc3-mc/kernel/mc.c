 ...
 *  の責任を負わない．
 *
 *  $Id: mc.c 485 2015-12-17 08:21:50Z witz-itoyo $
 */
...
ccb_initialize(void)
{
	...
	my_p_ccb->run_trusted = TRUE;

	/*
	 *  OSアプリケーション管理関係の初期化
	 */
	my_p_ccb->p_runosap = NULL;
	 ...
	 *  終了/再起動するOSAP関係の初期化
	 */
	my_p_ccb->term_osap_id_map = 0U;
	my_p_ccb->term_osap_opt_map = 0U;
}
...
ici_stk_monitoring(void)
{
	...
	ostk_top = _ostk_table[x_core_id()];
	if (ostk_top[0] != STACK_MAGIC_NUMBER) {
		/*
		 *  第1引数に1を設定して，プロテクションフック実行時に
		 *  スタック切り替えをする
		 *  第2引数にPEOB_STACKMONITORING_INTERRUPTを設定して，
		 *  ISRでのスタックモニタリングを示す
		 */
		stack_monitoring_error(1U);
	}
}
	...
	while (reqmap != 0U) {
		...
		if (icino < tnum_ici_of_core[my_p_ccb->coreid]) {
			...
			if ((my_p_ccb->ici_bit_mask & p_iciinib[icino].ici_bit_ptn) != 0U) {
				 ...
				 *  ICISRハンドラ実行中にこのICISRの所属OSAPをp_runosapに設定する
				 */
				p_runisr_saved = my_p_ccb->p_runisr;
				my_p_ccb->p_runisr = get_isrcb(((p_iciinb_table[my_p_ccb->coreid])[icino]).iciid);
				p_runosap_saved = my_p_ccb->p_runosap;
				my_p_ccb->p_runosap = my_p_ccb->p_runisr->p_isrinib->p_osapcb;
				...
				x_nested_lock_os_int();
				acquire_tsk_lock(my_p_ccb);
				my_p_ccb->p_runosap = p_runosap_saved;
