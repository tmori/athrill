 ...
 *  の責任を負わない．
 *
 *  $Id: interrupt_manage.c 440 2015-12-09 12:34:23Z aisincom-nishioka $
 */
...
SuspendAllInterrupts(void)
{
	...
	LOG_SUSALL_ENTER();
	S_N_CHECK_DISALLINT();
	S_N_CHECK_ERROR((p_ccb->run_trusted != FALSE), E_OS_ACCESS);
	/* ネスト回数の上限値超過 */
...
DisableInterruptSource(ISRType DisableISR)
{
  ...
  exit_errorhook:
	...
	get_my_p_ccb()->_errorhook_par1.isrid = DisableISR;
...
EnableInterruptSource(ISRType EnableISR)
{
  ...
  exit_errorhook:
	x_nested_lock_os_int();
#ifdef CFG_USE_PARAMETERACCESS
	get_my_p_ccb()->_errorhook_par1.isrid = EnableISR;
#endif /* CFG_USE_PARAMETERACCESS */
