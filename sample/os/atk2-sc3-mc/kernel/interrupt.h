...
/*
 *  ISRCBから割込み番号を割出すマクロ
 *  p_isrcbにはC2ISRの管理ブロックのみを想定
 */
#define GET_INTNO(p_isrcb)	((p_isrcb)->p_isrinib->p_intinib->intno)

/*
 *  ISRCBから割込み優先度を割出すマクロ
 *  p_isrcbにはC2ISRとICISRの管理ブロックを想定
 */
#define GET_INTPRI(p_isrcb)	((ISR2ID(p_isrcb) < (tnum_isr2 - tnum_ici)) ? \
							 ((p_isrcb)->p_isrinib->p_intinib->intpri) :  \
							 GET_ICI_INTPRI((p_isrcb)->p_isrinib->p_osapcb->p_osapinib->coreid))

/*
 *  ISRCBからコアIDを割出すマクロ
 */
#define GET_ISR_COREID(p_isrcb)	((p_isrcb)->p_isrinib->p_osapcb->p_osapinib->coreid)
