...
CheckTaskMemoryAccess(TaskType TaskID, MemoryStartAddressType Address, MemorySizeType Size)
{
  ...
  exit_errorhook:
	...
	get_my_p_ccb()->_errorhook_par2.adr = Address;
	get_my_p_ccb()->_errorhook_par3.sz = Size;
...
check_isr_memory(ISRCB *p_isrcb, const MemoryStartAddressType adr, MemorySizeType size, boolean *over_region)
{
	...
	if (size > memsize) {
		...
		*over_region = FALSE;
		access = check_address_stack(adr, size, _ostk_table[p_ccb->coreid], _ostksz_table[p_ccb->coreid]);
	}
...
probe_memory_access(const MemoryStartAddressType base, MemorySizeType size)
{
	...
	if (p_ccb->run_trusted != FALSE) {
		...
		access = check_task_memory(p_ccb->p_runtsk, base, size, &dummy);
	}
	return(access);
}
