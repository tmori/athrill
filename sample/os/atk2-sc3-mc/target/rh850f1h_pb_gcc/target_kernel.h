 ...
 *  の責任を負わない．
 *
 *  $Id: target_kernel.h 35 2014-07-17 14:00:37Z ertl-honda $
 */

/*
 *		Os.hのターゲット依存部（RH850F1H_PB用）
 *
 ...
 *  CoreのID定義
 */
#define OS_CORE_ID_0	UINT_C(0)
#define OS_CORE_ID_1	UINT_C(1)
...
/* チェック用非タスクコンテキスト用のスタック最小値の定義 */
#define MINIMUM_OSTKSZ	256

/* 各スタックサイズのデフォルト値の定義*/
#define DEFAULT_TASKSTKSZ	(1024U) /* 1K bytes */
#define DEFAULT_TASKSYSTEMSTKSZ			(1024U) /* 1K bytes */
#define DEFAULT_ISRSTKSZ	(1024U) /* 1K bytes */
#define DEFAULT_ISRSYSTEMSTKSZ			(1024U) /* 1K bytes */
#define DEFAULT_HOOKSTKSZ	(1024U) /* 1K bytes */
#define DEFAULT_TRUSTEDFUNCTIONSTKSZ	(128U)  /* 128 bytes */
 ...
 *  プロセッサで共通な定義
 */
#include "v850_gcc/prc_kernel.h"

#define TARGET_SVC_NUM	(PRC_SVC_NUM)
