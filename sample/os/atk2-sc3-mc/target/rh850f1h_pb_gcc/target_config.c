...
target_initialize(void)
{
	...
	if (coreid == 0) {
		for (j = 0U; j < tnum_intno; j++) {
			if (intinib_table[j].coreid == 1) {
				sil_wrw_mem((void *) (IBD_ADDRESS(INTNO_MASK(intinib_table[j].intno))), 0x00010002);
			}
		}
	}
}
...
target_exit(void)
{
	...
	p_ccb->target_ccb.p_ctxosap = NULL;
	/* 全コアが終了されるのを同期する */
	barrier_sync(6U, FALSE);
	if (x_core_id() == OS_CORE_ID_MASTER) {
        /* スレーブコアを停止 */
        core_state_table[1] = 0U;
    }
	barrier_sync(7U, FALSE);
	...
	if (x_core_id() == OS_CORE_ID_MASTER) {
		 ...
		 * コアが起動されるまで無限ループして待機する(slave_start)
		 */
		return_main_slave();
	}
