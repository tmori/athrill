 ...
 *  の責任を負わない．
 *
 *  $Id: trace_config.c 425 2015-12-07 08:06:19Z witz-itoyo $
 */
...
/*
 *  本来はtrace_config.hにプロトタイプ宣言を置くべきだが，TCBが
 *  定義される前なので，コンパイルが通らないためここに置く
 */
...
static TraceModeType	trace_mode = TRACE_STOP;            /* トレースモード初期化 */
...
trace_wri_log(TRACE *p_trace)
{
	...
	if (trace_mode != TRACE_STOP) {
		...
		trace_buffer[trace_tail].coreid = coreid;
		trace_tail++;
