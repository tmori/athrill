 ...
 *  の責任を負わない．
 *
 *  $Id: tool_svc.h 425 2015-12-07 08:06:19Z witz-itoyo $
 */
...
{
	CAL_SVC_0M(CoreIdType, TFN_GETCOREID);
}

LOCAL_INLINE uint32
_trap_GetNumberOfActivatedCores(void)
{
	CAL_SVC_0M(uint32, TFN_GETNUMBEROFACTIVATEDCORES);
}
...
{
	CAL_SVC_0M(StatusType, TFN_TERMINATETASK);
}

LOCAL_INLINE StatusType
_trap_ChainTask(TaskType TaskID)
{
	CAL_SVC_1M(StatusType, TFN_CHAINTASK, TaskType, TaskID);
}

LOCAL_INLINE StatusType
_trap_Schedule(void)
{
	CAL_SVC_0M(StatusType, TFN_SCHEDULE);
}

LOCAL_INLINE StatusType
_trap_GetTaskID(TaskRefType TaskID)
{
	CAL_SVC_1M(StatusType, TFN_GETTASKID, TaskRefType, TaskID);
}

LOCAL_INLINE StatusType
_trap_GetTaskState(TaskType TaskID, TaskStateRefType State)
{
	CAL_SVC_2M(StatusType, TFN_GETTASKSTATE, TaskType, TaskID, TaskStateRefType, State);
}
...
_trap_GetISRID(void)
{
	CAL_SVC_0M(ISRType, TFN_GETISRID);
}

LOCAL_INLINE StatusType
_trap_DisableInterruptSource(ISRType DisableISR)
{
	CAL_SVC_1M(StatusType, TFN_DISABLEINTERRUPTSOURCE, ISRType, DisableISR);
}

LOCAL_INLINE StatusType
_trap_EnableInterruptSource(ISRType EnableISR)
{
	CAL_SVC_1M(StatusType, TFN_ENABLEINTERRUPTSOURCE, ISRType, EnableISR);
}

LOCAL_INLINE StatusType
_trap_GetResource(ResourceType ResID)
{
	CAL_SVC_1M(StatusType, TFN_GETRESOURCE, ResourceType, ResID);
}

LOCAL_INLINE StatusType
_trap_ReleaseResource(ResourceType ResID)
{
	CAL_SVC_1M(StatusType, TFN_RELEASERESOURCE, ResourceType, ResID);
}

LOCAL_INLINE StatusType
_trap_SetEvent(TaskType TaskID, EventMaskType Mask)
{
	CAL_SVC_2M(StatusType, TFN_SETEVENT, TaskType, TaskID, EventMaskType, Mask);
}

LOCAL_INLINE StatusType
_trap_ClearEvent(EventMaskType Mask)
{
	CAL_SVC_1M(StatusType, TFN_CLEAREVENT, EventMaskType, Mask);
}

LOCAL_INLINE StatusType
_trap_GetEvent(TaskType TaskID, EventMaskRefType Event)
{
	CAL_SVC_2M(StatusType, TFN_GETEVENT, TaskType, TaskID, EventMaskRefType, Event);
}

LOCAL_INLINE StatusType
_trap_WaitEvent(EventMaskType Mask)
{
	CAL_SVC_1M(StatusType, TFN_WAITEVENT, EventMaskType, Mask);
}

LOCAL_INLINE StatusType
_trap_GetAlarmBase(AlarmType AlarmID, AlarmBaseRefType Info)
{
	CAL_SVC_2M(StatusType, TFN_GETALARMBASE, AlarmType, AlarmID, AlarmBaseRefType, Info);
}

LOCAL_INLINE StatusType
_trap_GetAlarm(AlarmType AlarmID, TickRefType Tick)
{
	CAL_SVC_2M(StatusType, TFN_GETALARM, AlarmType, AlarmID, TickRefType, Tick);
}

LOCAL_INLINE StatusType
_trap_SetRelAlarm(AlarmType AlarmID, TickType increment, TickType cycle)
{
	CAL_SVC_3M(StatusType, TFN_SETRELALARM, AlarmType, AlarmID, TickType, increment, TickType, cycle);
}

LOCAL_INLINE StatusType
_trap_SetAbsAlarm(AlarmType AlarmID, TickType start, TickType cycle)
{
	CAL_SVC_3M(StatusType, TFN_SETABSALARM, AlarmType, AlarmID, TickType, start, TickType, cycle);
}

LOCAL_INLINE StatusType
_trap_CancelAlarm(AlarmType AlarmID)
{
	CAL_SVC_1M(StatusType, TFN_CANCELALARM, AlarmType, AlarmID);
}

LOCAL_INLINE StatusType
_trap_IncrementCounter(CounterType CounterID)
{
	CAL_SVC_1M(StatusType, TFN_INCREMENTCOUNTER, CounterType, CounterID);
}

LOCAL_INLINE StatusType
_trap_GetCounterValue(CounterType CounterID, TickRefType Value)
{
	CAL_SVC_2M(StatusType, TFN_GETCOUNTERVALUE, CounterType, CounterID, TickRefType, Value);
}

LOCAL_INLINE StatusType
_trap_GetElapsedValue(CounterType CounterID, TickRefType Value, TickRefType ElapsedValue)
{
	CAL_SVC_3M(StatusType, TFN_GETELAPSEDVALUE, CounterType, CounterID, TickRefType, Value, TickRefType, ElapsedValue);
}

LOCAL_INLINE AccessType
_trap_CheckISRMemoryAccess(ISRType ISRID, MemoryStartAddressType Address, MemorySizeType Size)
{
	CAL_SVC_3M(AccessType, TFN_CHECKISRMEMORYACCESS, ISRType, ISRID, MemoryStartAddressType, Address, MemorySizeType, Size);
}

LOCAL_INLINE AccessType
_trap_CheckTaskMemoryAccess(TaskType TaskID, MemoryStartAddressType Address, MemorySizeType Size)
{
	CAL_SVC_3M(AccessType, TFN_CHECKTASKMEMORYACCESS, TaskType, TaskID, MemoryStartAddressType, Address, MemorySizeType, Size);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckTaskAccess(ApplicationType ApplID, TaskType TaskID)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKTASKACCESS, ApplicationType, ApplID, TaskType, TaskID);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckISRAccess(ApplicationType ApplID, ISRType ISRID)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKISRACCESS, ApplicationType, ApplID, ISRType, ISRID);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckAlarmAccess(ApplicationType ApplID, AlarmType AlarmID)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKALARMACCESS, ApplicationType, ApplID, AlarmType, AlarmID);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckResourceAccess(ApplicationType ApplID, ResourceType ResID)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKRESOURCEACCESS, ApplicationType, ApplID, ResourceType, ResID);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckCounterAccess(ApplicationType ApplID, CounterType CounterID)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKCOUNTERACCESS, ApplicationType, ApplID, CounterType, CounterID);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckScheduleTableAccess(ApplicationType ApplID, ScheduleTableType ScheduleTableID)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKSCHEDULETABLEACCESS, ApplicationType, ApplID, ScheduleTableType, ScheduleTableID);
}

LOCAL_INLINE ObjectAccessType
_trap_CheckSpinlockAccess(ApplicationType ApplID, SpinlockIdType SpinlockId)
{
	CAL_SVC_2M(ObjectAccessType, TFN_CHECKSPINLOCKACCESS, ApplicationType, ApplID, SpinlockIdType, SpinlockId);
}

LOCAL_INLINE ApplicationType
_trap_CheckTaskOwnership(TaskType TaskID)
{
	CAL_SVC_1M(ApplicationType, TFN_CHECKTASKOWNERSHIP, TaskType, TaskID);
}

LOCAL_INLINE ApplicationType
_trap_CheckISROwnership(ISRType ISRID)
{
	CAL_SVC_1M(ApplicationType, TFN_CHECKISROWNERSHIP, ISRType, ISRID);
}

LOCAL_INLINE ApplicationType
_trap_CheckAlarmOwnership(AlarmType AlarmID)
{
	CAL_SVC_1M(ApplicationType, TFN_CHECKALARMOWNERSHIP, AlarmType, AlarmID);
}

LOCAL_INLINE ApplicationType
_trap_CheckCounterOwnership(CounterType CounterID)
{
	CAL_SVC_1M(ApplicationType, TFN_CHECKCOUNTEROWNERSHIP, CounterType, CounterID);
}

LOCAL_INLINE ApplicationType
_trap_CheckScheduleTableOwnership(ScheduleTableType ScheduleTableID)
{
	CAL_SVC_1M(ApplicationType, TFN_CHECKSCHEDULETABLEOWNERSHIP, ScheduleTableType, ScheduleTableID);
}

LOCAL_INLINE ApplicationType
_trap_GetApplicationID(void)
{
	CAL_SVC_0M(ApplicationType, TFN_GETAPPLICATIONID);
}

LOCAL_INLINE StatusType
_trap_CallTrustedFunction(TrustedFunctionIndexType FunctionIndex, TrustedFunctionParameterRefType FunctionParams)
{
	CAL_SVC_2M(StatusType, TFN_CALLTRUSTEDFUNCTION, TrustedFunctionIndexType, FunctionIndex, TrustedFunctionParameterRefType, FunctionParams);
}

LOCAL_INLINE StatusType
_trap_GetApplicationState(ApplicationType Application, ApplicationStateRefType Value)
{
	CAL_SVC_2M(StatusType, TFN_GETAPPLICATIONSTATE, ApplicationType, Application, ApplicationStateRefType, Value);
}

LOCAL_INLINE AppModeType
_trap_GetActiveApplicationMode(void)
{
	CAL_SVC_0M(AppModeType, TFN_GETACTIVEAPPLICATIONMODE);
}

LOCAL_INLINE StatusType
_trap_StartScheduleTableRel(ScheduleTableType ScheduleTableID, TickType Offset)
{
	CAL_SVC_2M(StatusType, TFN_STARTSCHEDULETABLEREL, ScheduleTableType, ScheduleTableID, TickType, Offset);
}

LOCAL_INLINE StatusType
_trap_StartScheduleTableAbs(ScheduleTableType ScheduleTableID, TickType Start)
{
	CAL_SVC_2M(StatusType, TFN_STARTSCHEDULETABLEABS, ScheduleTableType, ScheduleTableID, TickType, Start);
}

LOCAL_INLINE StatusType
_trap_StopScheduleTable(ScheduleTableType ScheduleTableID)
{
	CAL_SVC_1M(StatusType, TFN_STOPSCHEDULETABLE, ScheduleTableType, ScheduleTableID);
}

LOCAL_INLINE StatusType
_trap_NextScheduleTable(ScheduleTableType ScheduleTableID_From, ScheduleTableType ScheduleTableID_To)
{
	CAL_SVC_2M(StatusType, TFN_NEXTSCHEDULETABLE, ScheduleTableType, ScheduleTableID_From, ScheduleTableType, ScheduleTableID_To);
}

LOCAL_INLINE StatusType
_trap_GetScheduleTableStatus(ScheduleTableType ScheduleTableID, ScheduleTableStatusRefType ScheduleStatus)
{
	CAL_SVC_2M(StatusType, TFN_GETSCHEDULETABLESTATUS, ScheduleTableType, ScheduleTableID, ScheduleTableStatusRefType, ScheduleStatus);
}

LOCAL_INLINE FaultyContextType
_trap_GetFaultyContext(void)
{
	CAL_SVC_0M(FaultyContextType, TFN_GETFAULTYCONTEXT);
}

LOCAL_INLINE StatusType
_trap_GetSpinlock(SpinlockIdType SpinlockId)
{
	CAL_SVC_1M(StatusType, TFN_GETSPINLOCK, SpinlockIdType, SpinlockId);
}

LOCAL_INLINE StatusType
_trap_ReleaseSpinlock(SpinlockIdType SpinlockId)
{
	CAL_SVC_1M(StatusType, TFN_RELEASESPINLOCK, SpinlockIdType, SpinlockId);
}

LOCAL_INLINE StatusType
_trap_TryToGetSpinlock(SpinlockIdType SpinlockId, TryToGetSpinlockType *Success)
{
	CAL_SVC_2M(StatusType, TFN_TRYTOGETSPINLOCK, SpinlockIdType, SpinlockId, TryToGetSpinlockType *, Success);
}

LOCAL_INLINE OSServiceIdType
_trap_get_error_svcid(void)
{
	CAL_SVC_0M(OSServiceIdType, TFN_GET_ERROR_SVCID);
}
...
{
	CAL_SVC_2M(Std_ReturnType, TFN_IOC_SEND_GENERIC, IocType, IocWrapperId, const void *, in);
}

LOCAL_INLINE Std_ReturnType
_trap_ioc_write_generic(IocType IocWrapperId, const void *in)
{
	CAL_SVC_2M(Std_ReturnType, TFN_IOC_WRITE_GENERIC, IocType, IocWrapperId, const void *, in);
}

LOCAL_INLINE Std_ReturnType
_trap_ioc_receive_generic(IocType IocId, void *out)
{
	CAL_SVC_2M(Std_ReturnType, TFN_IOC_RECEIVE_GENERIC, IocType, IocId, void *, out);
}

LOCAL_INLINE Std_ReturnType
_trap_ioc_read_generic(IocType IocId, void *out)
{
	CAL_SVC_2M(Std_ReturnType, TFN_IOC_READ_GENERIC, IocType, IocId, void *, out);
}

LOCAL_INLINE Std_ReturnType
_trap_ioc_empty_queue_generic(IocType IocId)
{
	CAL_SVC_1M(Std_ReturnType, TFN_IOC_EMPTY_QUEUE_GENERIC, IocType, IocId);
}

LOCAL_INLINE StatusType
_trap_RaiseInterCoreInterrupt(ISRType ISRID)
{
	CAL_SVC_1M(StatusType, TFN_RAISEINTERCOREINTERRUPT, ISRType, ISRID);
}

LOCAL_INLINE StatusType
_trap_AllowAccess(void)
{
	CAL_SVC_0M(StatusType, TFN_ALLOWACCESS);
}

LOCAL_INLINE StatusType
_trap_TerminateApplication(ApplicationType Application, RestartType RestartOption)
{
	CAL_SVC_2M(StatusType, TFN_TERMINATEAPPLICATION, ApplicationType, Application, RestartType, RestartOption);
}
