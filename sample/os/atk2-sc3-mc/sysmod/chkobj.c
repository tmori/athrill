 ...
 *  の責任を負わない．
 *
 *  $Id: chkobj.c 425 2015-12-07 08:06:19Z witz-itoyo $
 */
...
/*
 *  CheckObjectAccess()
 *   非信頼OSアプリケーションのオブジェクトアクセス可否をチェックする
 *
...
CheckObjectAccess(ApplicationType ApplID, ObjectTypeType ObjectType, ...)
{
	...
	switch (ObjectType) {
	case OBJECT_TASK:
		access = CheckTaskAccess(ApplID, object_id);
		break;

	case OBJECT_ISR:
		access = CheckISRAccess(ApplID, object_id);
		break;

	case OBJECT_ALARM:
		access = CheckAlarmAccess(ApplID, object_id);
		break;

	case OBJECT_RESOURCE:
		access = CheckResourceAccess(ApplID, object_id);
		break;

	case OBJECT_COUNTER:
		access = CheckCounterAccess(ApplID, object_id);
		break;

	case OBJECT_SCHEDULETABLE:
		access = CheckScheduleTableAccess(ApplID, object_id);
		break;

	case OBJECT_SPINLOCK:
		access = CheckSpinlockAccess(ApplID, object_id);
		break;
