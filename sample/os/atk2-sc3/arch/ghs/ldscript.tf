...
$      免責すること．
$
$  AUTOSAR（AUTomotive Open System ARchitecture）は，AUTOSAR仕様に基づ
$  いたソフトウェアを商用目的で利用する者に対して，AUTOSARパートナーに
$  なることを求めている．
$
...
$  の責任を負わない．
$
$  $Id: ldscript.tf 115 2014-12-10 01:33:17Z t_ishikawa $
$
...
$                $TAB$.rosdata : {$NL$
                ...
                $NL$$NL$
			$END$
$           // .idataセクションを配置するための記述
$           // およびアライン記述
			$IF !OMIT_IDATA && !EQ(MO.ILABEL[moid], "")$
                $TAB$.ROM_NOCOPY.$MO.SLABEL[moid]$ ROM(.$MO.SLABEL[moid]$): {$NL$
                $TAB$} > $REG.REGNAME[STANDARD_ROM]$$NL$
                $TAB$$PREFIX_START$$MO.ILABEL[moid]$ = ADDR(.ROM_NOCOPY.$MO.SLABEL[moid]$);$NL$
			$END$
...
$		// リンクするファイル記述の生成
		...
		$IF MO.TYPE[moid] == TOPPERS_ATTMOD$
			$IF EQ(MO.MODULE[moid], "kernel_mem.o")$
                $FOREACH sec section_dscr$
                    kernel_mem2.o($sec$)$SPC$
                $END$
			$ELSE$
				$module_name = REGEX_REPLACE(MO.MODULE[moid],"\.a$","\.a(\*")$
                $FOREACH sec section_dscr$
                    $module_name$($sec$)$SPC$
                    $IF !EQ(MO.MODULE[moid], module_name)$
                        )
                    $END$
                $END$
                $SPC$
...
$           // 中身が空の出力セクションはリンカによって無視されるため，
$           // ダミーのデータを入れておく
            $TAB$$TAB$. += 1; /*dummy*/$NL$
			$TAB$} > $REG.REGNAME[MO.MEMREG[moid]]$ $NL$
			$TAB$. = ALIGN($TARGET_SEC_ALIGN_STR$);$NL$
			$TAB$___$REG.REGNAME[MO.MEMREG[moid]]$_align = .;$NL$
			$NL$
		$END$
...
$FOREACH lsid RANGE(1, numls)$
	$TAB$$LNKSEC.SECTION[lsid]$ ALIGN(16) : {$NL$
...
$TAB$.dummy_rom_for_cpptest : {$NL$
$TAB$$TAB$*(.ctors.65535) *(.eh_frame)$NL$
$TAB$} > $REG.REGNAME[STANDARD_ROM]$$NL$
$TAB$. = ALIGN(64);$NL$$NL$
$TAB$.dummy_ram_for_cpptest : {$NL$
$TAB$$TAB$_end = .;$NL$
$TAB$} > $REG.REGNAME[STANDARD_RAM]$$NL$
$TAB$. = ALIGN(64);$NL$$NL$

$TAB$.tdata ALIGN (4) :$NL$
$TAB${$NL$
$TAB$	PROVIDE(__ep = .);$NL$
$TAB$	*(.tbyte)$NL$
$TAB$	*(.tcommon_byte)$NL$
