...
$
$  Copyright (C) 2007-2014 by Center for Embedded Computing Systems
...
$  の責任を負わない．
$
$  $Id: ldscript.tf 182 2015-06-24 07:14:37Z t_ishikawa $
$
...
/* This file is generated by v850_ghs/ldscript.tf */$NL$
...
$           // およびアライン記述
			...
			$IF (MO.SEFLAG[moid] & 0x40) == 0$
                $IF LENGTH(MO.REALBASE[moid])$
				    ($FORMAT("0x%x", MO.REALBASE[moid])$)$SPC$
                $ELSE$
				    ALIGN($TARGET_SEC_ALIGN_STR$)$SPC$
                $END$
			$ELIF LENGTH(MO.REALBASE[moid])$
                ($FORMAT("0x%x", MO.REALBASE[moid])$)$SPC$
			$END$
			...
			:$SPC$
			{
            $IF LENGTH(MO.COMMENT[moid])$
                $SPC$/* $MO.COMMENT[moid]$ */$SPC$
            $END$
            $NL$
		$END$
...
$		// アライン記述の生成
        $IF LENGTH(MO.REALALIGN[moid])$
			$TAB$. = ALIGN($FORMAT("0x%x", +MO.REALALIGN[moid])$); /* sdata_shared_align */$NL$
		$ELIF (MO.SEFLAG[moid] & 0x20) != 0$
...
$		// メモリリージョン末でアラインさせるためのダミーセクションの生成
		$IF (MO.SEFLAG[moid] & 0x80) != 0$
            ...
            $IF LENGTH(REG.LIMIT_ALIGN[MO.MEMREG[moid]])$
                ($FORMAT("0x%x", REG.LIMIT_ALIGN[MO.MEMREG[moid]])$)$SPC$
            $ELSE$
                ALIGN($TARGET_SEC_ALIGN_STR$)$SPC$
            $END$
            : {$NL$
