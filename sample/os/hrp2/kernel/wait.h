 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: wait.h 613 2012-05-13 00:32:16Z ertl-hiro $
 */
 ...
 *  タスク例外処理マスク状態か否かの判定
 *
 *  p_tcbで指定されるタスクが，タスク例外処理マスク状態の場合はtrue，そ
 *  うでない場合はfalseを返す．t_sense_texmaskは，タスクコンテキストか
 *  ら呼び出すため，i_sense_texmaskは，非タスクコンテキストから呼び出す
 *  ための関数である．
 */
Inline bool_t
t_sense_texmask(TCB *p_tcb)
{
