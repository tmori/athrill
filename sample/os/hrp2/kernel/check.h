 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: check.h 610 2012-05-06 10:26:14Z ertl-hiro $
 */
 ...
 *  オブジェクトIDのチェック（E_ID）
 */
#define CHECK_DOMID(domid) do {								\
	if (!VALID_DOMID(domid)) {								\
		ercd = E_ID;										\
		goto error_exit;									\
	}														\
} while (false)

#define CHECK_DOMID_SELF(domid) do {						\
	if (!(VALID_DOMID(domid) || (domid) == TDOM_SELF)) {	\
		ercd = E_ID;										\
		goto error_exit;									\
	}														\
} while (false)
