 ...
 *  の責任を負わない．
 * 
 *  $Id: task.h 837 2012-12-26 15:09:59Z ertl-hiro $
 */
 ...
 *  タスク管理ブロック（TCB）
 *
 *  HRPカーネルでは，タスクの起動要求キューイング数の最大値（TMAX_ACTCNT）
 ...
 *  		svclevel，bpriority，priority，wupque，enatex，waifbd，texptn，
 *			mutex_queue，leftotm
 *  ・待ち状態（二重待ち状態を含む）で有効：
 *  		p_winfo
 ...
 *  実行中の保護ドメインのビットパターン
 *
 *  実行中の処理単位が属する保護ドメインの情報を，ビットパターンの形で
 *  持つ変数．タスクコンテキストでのみ有効．
 *
 *  rundomは，タスク自身（タスク例外処理ルーチンを含み，拡張サービスコー
 *  ルルーチンを除く）を実行中は，p_runtsk->p_tinib->p_dominib->domptn
 *  に設定し，拡張サービスコールルーチンの実行中は，TACP_KERNELに設定す
 *  る．
 *
 *  rundomを書き換えるのは，ディスパッチャと拡張サービスコールの出入口
 *  処理のみである．
 */
extern ACPTN	rundom;
 ...
 *  ない．
 */
extern void	call_texrtn_stask(void);
 ...
 */
extern void	calltex_stask(void);
