 ...
 *  の責任を負わない．
 * 
 *  $Id: serial.c 837 2012-12-26 15:09:59Z ertl-hiro $
 */
 ...
 */
ER
_serial_serial_opn_por(ID portid)
{
	SPCB	*p_spcb;
	ER		ercd;
 ...
 */
ER
_serial_serial_cls_por(ID portid)
{
	SPCB	*p_spcb;
	ER		ercd;
	bool_t	eflag = false;
 ...
 */
ER_UINT
_serial_serial_wri_dat(ID portid, const char *buf, uint_t len)
{
	SPCB	*p_spcb;
	bool_t	buffer_full;
	uint_t	wricnt = 0U;
 ...
 */
ER_UINT
_serial_serial_rea_dat(ID portid, char *buf, uint_t len)
{
	SPCB	*p_spcb;
	bool_t	buffer_empty;
	uint_t	reacnt = 0U;
 ...
 */
ER
_serial_serial_ctl_por(ID portid, uint_t ioctl)
{
	SPCB	*p_spcb;
 ...
 */
ER
_serial_serial_ref_por(ID portid, T_SERIAL_RPOR *pk_rpor)
{
	SPCB	*p_spcb;
...
#ifndef PROBE_MEM_WRITE_LEN
#define PROBE_MEM_WRITE_LEN(p_var, type, len) \
				(ALIGN_TYPE(p_var, type) && (cdmid == TDOM_KERNEL		\
						|| prb_mem((void *)(p_var), len,				\
										 TSK_SELF, TPM_WRITE) == E_OK))
#endif /* PROBE_MEM_WRITE_LEN */

#ifndef PROBE_MEM_READ_LEN
#define PROBE_MEM_READ_LEN(p_var, type, len) \
				(ALIGN_TYPE(p_var, type) && (cdmid == TDOM_KERNEL		\
						|| prb_mem((void *)(p_var), len,				\
 ...
 */
ER_UINT
extsvc_serial_opn_por(intptr_t portid, intptr_t par2, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_serial_opn_por((ID) portid));
}

ER_UINT
extsvc_serial_cls_por(intptr_t portid, intptr_t par2, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_serial_cls_por((ID) portid));
}

ER_UINT
extsvc_serial_rea_dat(intptr_t portid, intptr_t buf, intptr_t len,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	ER		ercd;

	if (!PROBE_MEM_WRITE_LEN(buf, char, len)) {
		ercd = E_MACV;
	}
	else {
		ercd = _serial_serial_rea_dat((ID) portid, (char *) buf, (uint_t) len);
	}
	return((ER_UINT) ercd);
}

ER_UINT
extsvc_serial_wri_dat(intptr_t portid, intptr_t buf, intptr_t len,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	ER		ercd;

	if (!PROBE_MEM_READ_LEN(buf, char, len)) {
		ercd = E_MACV;
	}
	else {
		ercd = _serial_serial_wri_dat((ID) portid,
									(const char *) buf, (uint_t) len);
	}
	return((ER_UINT) ercd);
}

ER_UINT
extsvc_serial_ctl_por(intptr_t portid, intptr_t ioctl, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_serial_ctl_por((ID) portid, (uint_t) ioctl));
}

ER_UINT
extsvc_serial_ref_por(intptr_t portid, intptr_t pk_rpor, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	ER		ercd;

	if (!PROBE_MEM_WRITE(pk_rpor, T_SERIAL_RPOR)) {
		ercd = E_MACV;
	}
	else {
		ercd = _serial_serial_ref_por((ID) portid,
