 ...
 * 【テストの目的】
 *
 *  非タスクコンテキスト，割込ロック解除状態，CPUロック解除状態で発生し
 *  たCPU例外におけるシステム状態のテスト．割込み優先度マスク全解除状態，
 *  ディスパッチ禁止状態でテストする．
 *
 * 【テスト項目】
 *
 *  いずれも，非タスクコンテキスト，割込ロック解除状態，CPUロック解除状
 *  態で発生したCPU例外において，
 *
 ...
 *	== CPUEXC ==
 *	4:	state(true, false, true, true, false)				... (A)(C)
 *		assert(xsns_dpn(p_excinf) == true)					... (G)
 *	5:	HOOK(HOOK_POINT(%d))
 *				... CPU例外ハンドラからリターンできない場合はここで終了
 *		RETURN
 *	== ALM1（続き）==
 *	6:	state(true, false, true, true, false)				... (D)(F)
 *		DO(alm1_flag = true)
 *		RETURN	... CPUロック状態のままリターン
 *	== TASK1（続き）==
 *	7:	state(false, false, true, true, false)				... (E)
 *		ipm(TIPM_ENAALL)
 *	8:	END
 */
